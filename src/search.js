...
// Fixed: processing overlay on add, font-size 16px (no iOS zoom),
//        dynamic nutrition preview in favs panel
// ============================================================
...
function showProcessing() {
  if (document.getElementById('log-processing-overlay')) return;
  const el = document.createElement('div');
  el.id = 'log-processing-overlay';
  el.className = 'log-processing-overlay';
  el.innerHTML = '<div class="log-processing-overlay__spinner"></div>';
  document.body.appendChild(el);
}
function hideProcessing() {
  document.getElementById('log-processing-overlay')?.remove();
}
...
export async function openSearchSheet() {
  ...
  sheet.id = 'mobile-search-sheet'; sheet.className = 'bottom-sheet';
  sheet.innerHTML = `
...
function closeMobileSheet() {
  const sheet = document.getElementById('mobile-search-sheet'); if (!sheet) return;
...
async function runSheetSearch(rawQuery) {
  ...
  let filtered = !q ? foods.filter(f => favSet.has(f.no)) : foods.filter(f => f.name.toLowerCase().includes(q));
  filtered.sort((a,b)=>{const af=favSet.has(a.no)?0:1,bf=favSet.has(b.no)?0:1;return af!==bf?af-bf:a.name.localeCompare(b.name);});
  const container = document.getElementById('mobile-sheet-results'); if (!container) return;
  const results = filtered.slice(0, CONFIG.search.maxResults);
  if (!results.length) { container.innerHTML=`<p class="search-empty">${q?`No results for "${escHtml(q)}"` : 'Search for a food'}</p>`; return; }
...
function renderFavouritesPage() {
  const view = document.getElementById('view-favourites'); if (!view) return;
  const favSet   = store.state.favourites || new Set();
  ...
  return `
    <div class="favs-expand-inner" data-food-no="${food.no}">
      ...
      <input class="favs-expand__input" type="number"
        value="${lastAmt}" min="1" step="1"
        data-base="${food.amount}" data-food-no="${food.no}"
        aria-label="Amount" style="font-size:16px">
...
function bindFavouritesEvents(view) {
  ...
  view.querySelectorAll('.favs-row').forEach(row => {
    ...
    row.querySelector('.favs-row__star-btn')?.addEventListener('click', async e => {
      e.stopPropagation(); await removeFavourite(no, row);
    });
    // Click row top = expand
...
async function removeFavourite(foodNo, rowEl) {
  ...
  setTimeout(()=>{
    rowEl.remove();
    const remaining=(store.state.foods||[]).filter(f=>{const s=store.state.favourites;return s instanceof Set?s.has(f.no):Array.isArray(s)&&s.includes(f.no);});
...
// Live nutrition preview for expand panel
function updateFavExpandPreview(row, no) {
  ...
  const macrosEl = document.getElementById(`favs-macros-${no}`) || row.querySelector('.favs-expand__macros');
...
async function handleFavAdd(row, no) {
  ...
  // Show processing state — block all interactions
...
// ── Add sheet (iPhone) with live preview + processing ─────────
...
function buildPreviewChips(food, amount) {
  const ratio = amount/(food.amount||100);
  const cals  = Math.round(calcCalories(food.protein,food.carbs,food.fat)*ratio);
  return [['Cal',cals,''],['P',((food.protein||0)*ratio).toFixed(1),'g'],['C',((food.carbs||0)*ratio).toFixed(1),'g'],
          ['F',((food.fat||0)*ratio).toFixed(1),'g'],['Fi',((food.fibre||0)*ratio).toFixed(1),'g']]
...
async function handleToggleFavourite(foodNo, rowEl) {
  const favSet=store.state.favourites,wasFav=favSet.has(foodNo);
  ...
  catch(err){wasFav?favSet.add(foodNo):favSet.delete(foodNo);updateFavBtn(rowEl,wasFav);console.error('[search] handleToggleFavourite:',err);showToast('Failed to update favourite','error');}
}
function updateFavBtn(rowEl,isFav){const btn=rowEl.querySelector('.fav-btn');if(!btn)return;btn.classList.toggle('fav-btn--active',isFav);btn.setAttribute('aria-label',isFav?'Remove from favourites':'Add to favourites');}
